...
// ====== مسار المدير: جلب إعدادات أي مقهى ======
app.get('/api/admin/shop/:shopId/settings', authenticateAdmin, (req, res) => {
    const { shopId } = req.params;
    const data = readShops();
    const shop = data.shops.find(s => s.id === shopId);
...
app.post('/api/shop/settings', authenticateShop, (req, res) => {
    const { name, settings, googleMapsUrl, phone, address, workingHours, shopId: bodyShopId } = req.body;
    const data = readShops();
    
    let targetShopId = req.session.shopId;
    
    if (req.shop.isAdmin && bodyShopId && bodyShopId !== req.session.shopId) {
        targetShopId = bodyShopId;
    }
    
    const shop = data.shops.find(s => s.id === targetShopId);
    
    if (!shop) {
        return res.status(404).json({ success: false, error: 'Shop not found' });
    }
    
    if (name !== undefined && name.trim() !== '' && req.shop.isAdmin) {
        shop.name = name.trim();
    }
    ...
    if (googleMapsUrl !== undefined) shop.googleMapsUrl = googleMapsUrl;
    if (phone !== undefined) shop.phone = phone;
    if (address !== undefined) shop.address = address;
    if (workingHours !== undefined) shop.workingHours = workingHours;
    
    writeShops(data);
    
    io.to(`shop_${shop.id}`).emit('settings-updated', shop.settings);
    io.to(`shop_${shop.id}`).emit('shop-updated', { name: shop.name });
    
    res.json({ success: true, shop });
});

app.get('/api/shop/settings', authenticateShop, (req, res) => {
    const shop = req.shop;
    res.json({ 
        success: true, 
        settings: shop.settings || {
